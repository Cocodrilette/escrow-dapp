...
export class Deployer {
  ...
  constructor(
    ...
    private readonly reactComponentsGenerator?: ReactComponentGenerator
  ) {}
  ...
  async deploy(): Promise<void> {
    try {
      ...
      const contracts = contractsNames.map(async (contractName: string) => {
        ...
        if (
          factory.interface.fragments[0].type === "constructor" &&
          factory.interface.fragments[0].inputs.length > 0
        ) {
